...
do
  ...
  vim.api.nvim_create_autocmd('PackChanged', {
    callback = function(ev)
      ...
      if name == 'kitty-navigator.nvim' then
        local kitty_config_dir = (vim.env.XDG_CONFIG_HOME or vim.env.HOME .. '/.config') .. '/kitty'
        run_build(name, { 'cp', 'navigate_kitty.py', 'pass_keys.py', kitty_config_dir }, ev.data.path)
        return
      end
    end,
  })
...
do
  ...
  -- ~/base16-kitty/colors/south.conf (see EXTRA_COLORSCHEMES below).
  vim.pack.add { gh 'RRethy/nvim-base16', gh 'arnauKL/south.nvim' }

  -- Non-base16 colorschemes that still have a matching
  -- ~/base16-kitty/colors/<name>.conf and so can be synced to Kitty and
  -- shown in the <leader>c picker.
  local EXTRA_COLORSCHEMES = { 'south' }

  local base16_theme_fname = vim.fn.expand((vim.env.XDG_CONFIG_HOME or vim.env.HOME .. '/.config') .. '/.base16_theme')
  ...
  -- south.nvim's `Cursor` group (which MiniStatuslineModeNormal links to by
  -- default) only sets a background, no foreground, so it falls back to
  -- Normal's fg -- which is the exact same dark slate as Cursor's bg. That
  -- makes the "NORMAL" mode label render as invisible text on itself.
  -- Override it with an explicit, legible foreground.
  vim.api.nvim_set_hl(0, 'MiniStatuslineModeNormal', { fg = '#fcfcfd', bg = '#323b45', bold = true })
...
do
  ...
  -- Live-preview base16 colorschemes (synced to Kitty) while scrolling, commit on select.
  vim.keymap.set('n', '<leader>c', function()
    local colors = vim.fn.getcompletion('base16', 'color')
    vim.list_extend(colors, EXTRA_COLORSCHEMES)
      ...
      .new(theme, {
        ...
        attach_mappings = function(bufnr)
          ...
          telescope_action_set.shift_selection:enhance {
            post = function() vim.cmd.colorscheme(action_state.get_selected_entry().value) end,
          }
          return true
        end,
      })
